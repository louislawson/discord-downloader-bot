...
- `downloader_bot/worker/main.py` — defines `WorkerSettings` (the ARQ entrypoint discovered by import path) and `on_startup`/`on_shutdown` hooks. `on_startup` opens a REST-only Discord client, an asyncpg pool, and an `aiohttp.ClientSession` (timeout 1800s total / 60s sock_read; connector limit 20, limit_per_host 10) for CDN downloads, stashing all three on `ctx` (`ctx['discord_client']`, `ctx['db_pool']`, `ctx['http']`). `WorkerSettings.functions` registers two jobs: `download_channel_media` (the real pipeline) and `noop_job` (an echo-back smoke test the `<PREFIX>queueping` command enqueues). `max_jobs=3`, `job_timeout=1800` (30 min), `max_tries=2` (only honoured for `Retry`/`RetryJob`-driven retries).
